#!/usr/bin/env python3
# bum.py — Best Practice Codebase Aggregator (Express / Prisma / Node.js Backend)
import os
import sys
import re
import argparse
...
# Menyimpan seluruh konfigurasi penapisan secara terpusat untuk Express & Node.js
# =========================================================================
class AggregatorConfig:
    ...
    FORBIDDEN_FILES = {
        ...
        ".DS_Store", "Thumbs.db", "skills-lock.json"
    }

    # 3. Whitelist Folder yang Diizinkan untuk Ditelusuri (Express & Node.js Backend)
    ALLOWED_DIRS = {
        "src", "prisma", "config", "controllers", "services", "routes",
        "workers", "jobs", "middlewares", "repositories", "interfaces",
        "parsers", "providers", "dtos", "dto", "entities", "utils",
        "helpers", "models", "validators", "queues", "migrations",
        "seeders", "filters", "pipes", "interceptors", "guards",
        "decorators", "adapters", "factories", "types", "enums", "common"
    }

    # 4. Ekstensi Berkas Teks yang Diizinkan untuk Backend JavaScript / Prisma
    INCLUDE_EXTENSIONS = {
        ".ts", ".tsx", ".js", ".mjs", ".cjs", ".json", ".prisma",
        ".yaml", ".yml", ".md", ".env.example", ".sh", ".sql"
    }

    # 5. Berkas Konfigurasi Wajib di Tingkat Root
    ESSENTIAL_ROOT_FILES = {
        "package.json", "tsconfig.json", "docker-compose.yml",
        "Dockerfile", ".env.example", "schema.prisma", "index.js", "server.js",
        "app.js", "nodemon.json", "jest.config.js", ".eslintrc.js", ".eslintrc.json",
...
class CodebaseAggregator:
    ...
    def execute(self):
        ...
        if self.strip_comments:
            print("✂️ Pembersihan komentar & JSDoc diaktifkan untuk berkas JS/Prisma.")
        if self.exclude_tests:
        ...
        try:
            with open(self.output_file, "w", encoding="utf-8") as out_file:
                out_file.write("=== MARS-BE SMART ANALYSIS (BACKEND CODEBASE BUNDLE) ===\n\n")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Optimasi Bundel Kode Backend Express / Prisma / Node.js")
